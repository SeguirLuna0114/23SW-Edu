...
#공공데이터포털에서 'BusanHospital'데이터를 가져와 JSON 형식으로 저장
def getHospitalData(pageNo, numOfRows):
    # pageNo와 numOfRows를 매개변수로 받음
    end_point = 'http://apis.data.go.kr/6260000/MedicInstitService/MedicalInstitInfo' #데이터를 가져올 API의 엔드포인트 URL

    #API 호출에 필요한 파라미터들을 저장하기 위함
    parameters = ''
    parameters += "?resultType=json" # JSON 형식으로 요청
...
while(True): #무한루프
    ...
    jsonData = getHospitalData(pageNo, numOfRows) #getHospitalData(pageNo, numOfRows) 함수를 호출하여 데이터를 가져옴
    print(jsonData)

    if (jsonData['MedicalInstitInfo']['header']['resultCode'] == '00'): #jsonData의 결과 코드를 확인하여 성공적으로 데이터를 가져왔는지 확인
    ...
    savedFilename = 'xx_Busan_medical.json' #저장할 파일 명 설정
    
    #jsonResult에 저장된 데이터를 JSON 형식으로 변환하여 파일에 저장
    #with 문을 사용 => 파일을 열고 사용한 후에 자동으로 닫히도록 보장
    with open(savedFilename, 'w', encoding='utf8') as outfile: #파일을 쓰기 모드로 열기
