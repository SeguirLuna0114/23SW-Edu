...
@app.get('/getMongoCollectionData_30')
async def getMongoCollectionData_30(collectionName: str = None):
    get_mycol = mydb[collectionName] # 컬렉션을 선택
    return list(get_mycol.find().limit(30))

@app.get('/getMongoCollectionData_ALL')
async def getMongoCollectionData_ALL(collectionName: str = None):
    get_mycol = mydb[collectionName] # 컬렉션을 선택
...
#url을 활용하여 데이터 불러오기
@app.get('/getData_UlfptcaAlarmInfo_year')
async def getData_UlfptcaAlarmInfo_year(year: int = None):
...
@app.get('/getData_UlfptcaAlarmInfo_ALLYear')
async def getData_UlfptcaAlarmInfo_ALLYear():
    for year in range(2018, 2024):
        ...
        for item in data_dict['response']['body']['items']:
            ...
            return  {'error': 'Failed to receive response from API endpoint.'}
    return result

#mongodb에 Import
@app.get('/mongoImport_JSON')
async def mongoImport_JSON():
    Importcollection = mydb['AllParticleDatasTEST']
    ...
    Importcollection.insert_many(result)
    print(result)
